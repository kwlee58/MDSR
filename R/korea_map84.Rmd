...
shp_simp84$id <- rownames(shp_simp84@data) #> `id` 변수 생성
shp_simp84@data #> 데이터 슬롯 확인
map_merged <- merge(map_simp, shp_simp84@data, by = "id") #> shp_simp의 데이터 슬롯을 `map_simp` 에 통합.
str(map_merged) #> 데이터 구조 확인. `region` 없는 것 확인
map_merged$region <- map_merged$code #> region 변수 생성
str(map_merged) #> `region` 생성 확인.
map_coords <- coordinates(shp_simp84) #> 필요한 좌표 복사
...
```{r, eval = TRUE}
ggplot(data = map_df,  #> map_simp 로 돌리면 "discrete value supplied to continuous scale" 에러가 발생하는 데 inline 코드로 실행시키면 문제 없이 실행됨.
       mapping = aes(x = long,
                     y = lat,
                     group = group)) +
  geom_polygon(fill = "white",
